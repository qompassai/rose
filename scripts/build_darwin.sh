...
export GOFLAGS="'-ldflags=-w -s \"-X=github.com/qompassai/rose/version.Version=${VERSION#v}\" \"-X=github.com/qompassai/rose/server.mode=release\"'"
...
_sign_darwin() {
    ...
    mkdir -p dist/darwin
    lipo -create -output dist/darwin/rose dist/darwin-*/rose
    chmod +x dist/darwin/rose

    if [ -n "$APPLE_IDENTITY" ]; then
        for F in dist/darwin/rose dist/darwin-amd64/lib/rose/*; do
            codesign -f --timestamp -s "$APPLE_IDENTITY" --identifier ai.qompass.rose --options=runtime $F
        done
    ...
    tar -cf dist/rose-darwin.tar --strip-components 2 dist/darwin/rose
    tar -rf dist/rose-darwin.tar --strip-components 4 dist/darwin-amd64/lib/
...
_build_macapp() {
    ...
    mv ./macapp/out/make/zip/darwin/universal/Rose-darwin-universal-$VERSION.zip dist/Rose-darwin.zip
}
