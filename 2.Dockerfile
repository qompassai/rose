...
# Base stage for AMD64 architecture
...
# Base stage for ARM64 architecture
...
# Common base stage that uses the architecture-specific base
...
# JetPack 5 build (ARM64 only)
FROM nvcr.io/nvidia/l4t-jetpack:${JETPACK5VERSION} AS jetpack-5
...
# JetPack 6 build (ARM64 only)
FROM nvcr.io/nvidia/l4t-jetpack:${JETPACK6VERSION} AS jetpack-6
ARG CMAKEVERSION
RUN apt-get update && apt-get install -y curl ccache \
	&& curl -fsSL https://github.com/Kitware/CMake/releases/download/v${CMAKEVERSION}/cmake-${CMAKEVERSION}-linux-$(uname -m).tar.gz | tar xz -C /usr/local --strip-components 1
COPY CMakeLists.txt CMakePresets.json .
...
# ARM64 specific archive with JetPack support
...
# ROCm specific archive
FROM scratch AS rocm
COPY --from=rocm-6 dist/lib/rose/rocm /lib/rose/rocm

# Consolidation stage - choose the right components based on TARGETARCH
